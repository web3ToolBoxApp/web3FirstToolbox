...
合约 `iW3ft` 继承自 `IERC20Metadata` 接口，并定义了一些特定的函数：

- `mint(address to, uint256 amount)`：铸造指定数量的代币并发送到目标地址。
- `mint(address[] calldata to, uint256[] calldata amount)`：批量铸造不同数量的代币并发送到目标地址数组。
- `burn(uint256 amount)`：销毁指定数量的代币。
- `transferOwnership(address newOwner)`：将代币的所有权转移给新的拥有者。
...
- `buyTool(uint w3ftAmount, uint itemId)`：使用指定数量的代币购买工具的函数，通过调用 `token` 合约的转账函数实现。
...
- `Loan(address who)`：贷款成功时触发的事件。
- `PayBack(address who)`：还款成功时触发的事件。
...
- `getExpiredTime(uint tokenId)：返回指定 NFT 的到期时间。
...
- `burn(uint256[] calldata tokenIds)：烧毁多个指定的 NFT。
- `burn(uint256 tokenId)：烧毁指定的 NFT。
...
ShareDividend 合约是一个用于分红的合约，基于 ERC721Enumerable 标准，并继承了 Ownable 合约。
...
- `emptyProfit(uint round)`：将上一轮的利润转移至当前轮。
...
- `withdrawShareEvent(address who, uint avaliableShare, uint shareAmount)`：当所有者提取股份时触发的事件，包括所有者地址、可用股份数量和提取的股份数量。
